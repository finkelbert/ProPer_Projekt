...
##### Periodic energy (smog_pp)
  geom_line(aes(y=smog_pp_rel*f0_stim_min),color="red", alpha=.5, size=1.5) +
#### boundaries
...
#### boundaries
  ...
  geom_segment(aes(x=CoM_corr, xend=CoM_corr, y=60, yend=f0_stim_min), position = "dodge", color="green", size=.3, alpha=1, linetype = "dotted", lineend = "round") + 
...
#### annotations
  ...
  geom_text(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-5, y=50, label="Scaling (difference in Hz):"), check_overlap=T, color="black", size=3, family= "Helvetica", hjust="right") + 
  geom_text(aes(x=single_stim$CoM, y=50, label=round(scaling)), check_overlap=T, color="black", size=3, family= "Helvetica") + 
  geom_text(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-5, y=30, label="Synchrony (distance in ms):"), check_overlap=T, color="black", size=3, family= "Helvetica", hjust="right") + 
  geom_text(aes(x=single_stim$CoM, y=30, label=round(synchrony)), check_overlap=T, color="black", size=3, family= "Helvetica") +
  geom_text(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-5, y=10, label="Mass (relative AUC):"), check_overlap=T, color="black", size=3, family= "Helvetica", hjust="right") + 
...
#### legendary
  geom_segment(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-10, xend=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-10, y=65, yend=85), color="red", size=.3, alpha=.8, linetype = "dashed", lineend = "round", position = "dodge") +
  geom_text(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-20, y=75), label="CoM", color="red", alpha=1, size=3, family = "Helvetica", check_overlap=T, na.rm = T, hjust="right") +
  geom_segment(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-10, xend=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-10, y=90, yend=110), color="blue", size=.3, alpha=.8, linetype = "dotdash", lineend = "round", position = "dodge") +
  geom_text(aes(x=min(single_stim$bound,single_stim$interval_bounds,na.rm = T)-20, y=100), label="CoG", color="blue", alpha=1, size=3, family = "Helvetica", check_overlap=T, na.rm = T, hjust="right") +
#### plot stuff
