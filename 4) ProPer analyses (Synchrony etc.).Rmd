...
##  f0_smooth_stretch_interp_smooth_drop interpolation
  f0Floor4real = ifelse(
  smogPP_20Hz < 0.1, NA, f0_ed_dropPol)
  # f0_ed_dropPol = ifelse(
...
CoG <- function(df) {
  ...
  f0Floor <- ifelse(df$f0_speaker_range_median > 99, 
                    min(df$f0Floor4real, na.rm=T) - .1 * df$f0_speaker_range_median, 
...
####################### F0 curves
  ...
  # geom_point(aes(y=f0_ed_dropPol),color="purple", alpha=1, size=.3) +
  ##### periogram (smogPP)
