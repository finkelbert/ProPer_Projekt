...
#
### prepare butterworth low-pass filters
LowPass40Hz <- butter(n = 2, W = 40/(1000/2), type = "low")
LowPass20Hz <- butter(n = 2, W = 20/(1000/2), type = "low")
LowPass2Hz <- butter(n = 1, W = 2/(1000/2), type = "low")
# LowPassDyn <- butter(n = 2, W = detection_sensitivity/(1000/2), type = "low")
#
...
## compute and smooth 1st and 2nd derivatives of the periodic energy curve
  smogPP_1stDer = ifelse(t==0, 0, diff(smogPP_20Hz)*1000), # ifelse(t==0, 0, (smogPP_20Hz - lag(smogPP_20Hz,1))*1000),
  # smogPP_1stDer = ifelse(t==0, 0, (smogPP_20Hz - lag(smogPP_20Hz,1))*1000),
  smogPP_1stDer = filter(LowPass40Hz, smogPP_1stDer),
  # smogPP_1stDer = bwfilter(wave = smogPP_1stDer, f = 1000, to = 40, n = 2),
  smogPP_2ndDer = ifelse(t==0, 0, diff(smogPP_1stDer)*20) # ifelse(t==0, 0, (smogPP_1stDer - lag(smogPP_1stDer,1))*20)
...
trySensitivity <- function(ee, detection_sensitivity) {
  ...
  mutate(group_by(ee$single_token),
         smogPP_2ndDerDyn = # filter(butter(n = 2, W = detection_sensitivity/(1000/2), type = "low”), smogPP_2ndDer)
           bwfilter(
           wave = smogPP_2ndDer,
           f = 1000,
...
comp_df <- mutate(
  group_by(comp_df, file),
##  f0_smooth_stretch_interp_smooth_drop interpolation
  f0_smootcherpooth_dropPol = ifelse(
  is.na(f0_smooth) | smogPP_20Hz < 0.1, NA, f0_smootcherpooth)
# ### stretched F0 time-series with "real' local minima (used to compute CoG)
#   f0_realFloorStretch = ifelse(smogPP_20Hz > 0.1, f0_smootcherpooth, NA),
...
# CoG uses the interpolated F0 curve ('f0_smootcherpooth') multiplied by a 0--1 scale of the corresponding periodic energy  ('smogPP_20Hz')
CoG <- function(df) {
  # set the floor of the F0 curve: the interval's minimum minus a fixed percentage of the overall range (.1--.25)
  f0Floor <- ifelse(df$f0_speaker_range_median > 99, 
                    min(df$f0_smootcherpooth_dropPol, na.rm=T) - .1 * df$f0_speaker_range_median, 
                    min(df$f0_smootcherpooth_dropPol, na.rm=T) - .1 * 100)
                    # min(df$f0_realFloorStretch, na.rm=T) - .1 * df$f0_speaker_range_median, 
                    # min(df$f0_realFloorStretch, na.rm=T) - .1 * 100)
  ifelse(
    sum(df$smogPP_20Hz, na.rm=T) < 5, 0, # minimum = 50 (ms) * 0.1 (smogPP) = 5
    round(sum((df$f0_smootcherpooth - f0Floor) * df$t * df$smogPP_20Hz, na.rm=T) / sum((df$f0_smootcherpooth - f0Floor) * df$smogPP_20Hz, na.rm=T),3))
  }
#
# F0 and periodic energy values at centers
f0atCoM <- function(df) ifelse(df$CoM==0, 0, df$f0_smootcherpooth[df$t == round(df$CoM)])
f0atCoG <- function(df) ifelse(df$CoG==0, 0, df$f0_smootcherpooth[df$t == round(df$CoG)])
...
comp_df <- mutate(
  ...
  DeltaF0_rel = round((DeltaF0 / f0_speaker_range_median) * 100, 3),
...
####################### F0 curves
  ...
  ##### interpolated
  # geom_point(aes(y=f0_smootcherpooth),color="red", alpha=.3, size=.3) +
  # geom_point(aes(y=f0_smootcherpooth_dropPol),color="purple", alpha=1, size=.3) +
  ##### periogram (smogPP)
  geom_line(aes(y=f0_smootcherpooth),color="blue", alpha=single_token$smogPP_20Hz, size=single_token$smogPP_20Hz*3.5) +
...
####################### annotations
  ...
  ##### Synchrony
  geom_text(aes(x=CoM_corr + (round(synchrony)/2), y=f0atCoG-plotUnits*4, label=syncLabel), check_overlap=T, color="darkorchid", size=2, family= "Helvetica") +
  ##### Mass
...
####################### legend
  ...
  ##### Synchrony
  geom_text(aes(x=maxBounds+10, y=midHigh-plotUnits*2), label="Synchrony", check_overlap=T, color="darkorchid", size=2.5, family= "Helvetica", hjust="left") +
  geom_text(aes(x=maxBounds+10, y=midHigh-plotUnits*4), label="fall < 0 < rise", check_overlap=T, color="darkorchid", size=2, family= "Helvetica", hjust="left") +
  ##### MAss
...
mini_comp_df <- droplevels(subset(comp_df, select = -c(plotFloorToken, plotFloorSpeaker, plotFloorData, intensityRel, postPP_rel, logPP_rel, smogPP_1stDer, smogPP_2ndDer, smogPP_2ndDerDyn, CoM_seq, CoM_diff, CoM_diff_rel, CoM_diff_relInv, CoM_diff_relInterp, CoM_diff_relInterpStretch, CoM_diff_relSmoothInterp)))
