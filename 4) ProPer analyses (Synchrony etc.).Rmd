...
##  f0_smooth_stretch_interp_smooth_drop interpolation
  f0Floor4real = ifelse(
  smogPP_20Hz < 0.1, NA, f0Post_dropInterp)
  # f0Post_dropInterp = ifelse(
  # is.na(f0_smooth) | smogPP_20Hz < 0.1, NA, f0Post)
# ### stretched F0 time-series with "real' local minima (used to compute CoG)
#   f0_realFloorStretch = ifelse(smogPP_20Hz > 0.1, f0Post, NA),
...
# CoG uses the interpolated F0 curve ('f0Post') multiplied by a 0--1 scale of the corresponding periodic energy  ('smogPP_20Hz')
CoG <- function(df) {
  ...
  ifelse(
    sum(df$smogPP_20Hz, na.rm=T) < 5, 0, # minimum = 50 (ms) * 0.1 (smogPP) = 5
    round(sum((df$f0Post - f0Floor) * df$t * df$smogPP_20Hz, na.rm=T) / sum((df$f0Post - f0Floor) * df$smogPP_20Hz, na.rm=T),3))
  }
#
# F0 and periodic energy values at centers
f0atCoM <- function(df) ifelse(df$CoM==0, 0, df$f0Post[df$t == round(df$CoM)])
f0atCoG <- function(df) ifelse(df$CoG==0, 0, df$f0Post[df$t == round(df$CoG)])
...
####################### F0 curves
  ...
  ##### periogram (smogPP)
  geom_line(aes(y=f0Post),color="blue", alpha=single_token$smogPP_20Hz, size=single_token$smogPP_20Hz*3.5) +
