...
## comment out below this line if there's no manual segmetation:
    andCond(
...
## comment out up to this line if there's no manual segmetation:
    which()
}
#
...
if(useManual==TRUE) comp_df <- mutate(
  ...
  auto_bounds = ifelse(
    (!is.na(syll_bounds) & syll_bounds > auto_boundsSeq +autoMan & syll_bounds > lead(auto_boundsSeq, autoMan)),
...
CoG <- function(df) {
  ...
  f0Floor <- min(df$f0_interp_stretch_smooth, na.rm=T) - .2 * df$f0_token_range 
  ifelse(
    sum(df$smogPP_20Hz, na.rm=T) < 5, 0, # minimum = 50 (ms) * 0.1 (smogPP) = 5
...
plotUnits <- round(single_token$f0_token_range[1]/30)
# if(useManual==TRUE)
...
midLow <- round(single_token$f0_token_min[1]-single_token$f0_token_range[1]/2)
midHigh <- round(single_token$f0_token_max[1]-single_token$f0_token_range[1]/2)
...
####################### Periodic energy curve
  geom_line(aes(y=smogPP_20Hz*f0_token_range+plotFloor),color="red", alpha=.75, size=.5) +
####################### Speech-rate curve
  geom_line(aes(y=localSpeechRate*f0_token_range+plotFloor),color="springgreen4", alpha=.75, size=2.5, linetype="solid", linejoin = "round")+#, lineend = "butt", linejoin = "round") +
####################### boundaries
  {if(manual==TRUE) geom_segment(aes(x=syll_bounds, xend=syll_bounds, y=plotFloor, yend=f0_token_max+plotUnits*9), linetype="dotted", color="black", size=.5, alpha=.7)} +
  geom_segment(aes(x=auto_bounds, xend=auto_bounds, y=plotFloor, yend=f0_token_max+plotUnits*9), position = "dodge", color="red", size=.5, alpha=.5, linetype = "solid", lineend = "round") +
####################### landmarks 
  ##### CoMs
  geom_segment(aes(x=CoM_corr, xend=CoM_corr, y=plotFloor, yend=PERatCoM*f0_token_range+plotFloor), position = "dodge", color="red", size=.5, alpha=.5, linetype = "longdash", lineend = "round") +
...
####################### annotations
  ##### text
  {if(manual==TRUE) geom_text(aes(x=single_token$syll_mid, y=f0_token_max+plotUnits*8, label=syll_label, check_overlap=T), color="black", size=3, family= "Helvetica")} +
  ##### ∆F0 / F0 @ CoM
...
####################### plot stuff
  ...
  ylim(single_token$plotFloor[1]-plotUnits, single_token$f0_token_max[1]+plotUnits*9) +
...
mini_comp_df <- comp_df[c("file", "t", "speaker", 
                          if(manual==TRUE) "syll_mid", 
                          if(manual==TRUE) "syll_bounds", 
                          if(manual==TRUE) "syll_label", 
                          if(length(comp_df$word_bounds) > 0) "word_mid", 
                          if(length(comp_df$word_bounds) > 0) "word_bounds", 
                          if(length(comp_df$word_bounds) > 0) "word_label", 
                          "f0_speaker_min", "f0_speaker_max", "f0_speaker_median", "f0_speaker_mean", "f0_speaker_range", "f0_token_min", "f0_token_max", "f0_token_median", "f0_token_mean", "f0_token_range", "smogPP_20Hz", "smogPP_12Hz", "smogPP_8Hz", "smogPP_5Hz", "f0_interp_stretch_smooth", "f0_interp_smooth", 
                          if(useManual==TRUE) "syll_boundsSeq", 
                          "auto_bounds", "DeltaF0", 
                          if(useManual==TRUE) "auto_boundsSeq", 
                          "mass", "intervalDuration", "CoM", "CoG", "f0atCoM", "PERatCoM", "f0atCoG", "PERatCoM", "PERatCoG", "synchrony", "CoM_corr", "CoG_corr", "DeltaF0_rel", "sync_rel", "mass_rel", "intervalDuration_rel", "DeltaF0Label", "syncLabel", "CoM_seq", "localSpeechRate")]
