...
mkBoundetect <- function(single_stim) {
  ...
  TRUE %>%
    ...
    andCond(
      ...
      single_stim$t < single_stim$syll_boundsSeq +autoMan |
      ## determine right distance (in ms) from manual boundaries
      single_stim$t < lead(single_stim$syll_boundsSeq, autoMan)
    ) %>%
    which()
}
#
...
comp_df <- mutate(
  ...
  ## '∆F0': add stimulus-initial values (realtive to speaker's median F0)
  DeltaF0 = ifelse(
