...
CoG <- function(df) {
  ...
  f0Floor <- ifelse(df$f0_speaker_range > 100, 
                    min(df$f0_interp_stretch_smooth, na.rm=T) - .1 * df$f0_speaker_range, 
                    min(df$f0_interp_stretch_smooth, na.rm=T) - 10)
  ifelse(
    sum(df$smogPP_20Hz, na.rm=T) < 5, 0, # minimum = 50 (ms) * 0.1 (smogPP) = 5
...
ySclae <- c('tokenScale', 'speakerScale', 'dataScale')[1]
...
plotFloor <- ifelse(ySclae == 'tokenScale', single_token$plotFloorToken[1],
                     ifelse(ySclae == 'speakerScale', single_token$plotFloorSpeaker[1],
                            ifelse(ySclae == 'dataScale', single_token$plotFloorData[1], -275)))
plotUnits <- ifelse(ySclae == 'tokenScale', round(single_token$f0_token_range[1]/30),
                     ifelse(ySclae == 'speakerScale', round(single_token$f0_speaker_range[1]/30),
                            ifelse(ySclae == 'dataScale', round(single_token$f0_data_range[1]/30), 12)))
f0range <- ifelse(ySclae == 'tokenScale', single_token$f0_token_range[1],
                     ifelse(ySclae == 'speakerScale', single_token$f0_speaker_range[1],
                            ifelse(ySclae == 'dataScale', single_token$f0_data_range[1], 350)))
f0max <- ifelse(ySclae == 'tokenScale', single_token$f0_token_max[1],
                     ifelse(ySclae == 'speakerScale', single_token$f0_speaker_max[1],
                            ifelse(ySclae == 'dataScale', single_token$f0_data_max[1], 425)))
midLow <- ifelse(ySclae == 'tokenScale', round(single_token$f0_token_min[1]-single_token$f0_token_range[1]/2),
                     ifelse(ySclae == 'speakerScale', round(single_token$f0_speaker_min[1]-single_token$f0_speaker_range[1]/2),
                            ifelse(ySclae == 'dataScale', round(single_token$f0_data_min[1]-single_token$f0_data_range[1]/2), -100)))
midHigh <- ifelse(ySclae == 'tokenScale', round(single_token$f0_token_max[1]-single_token$f0_token_range[1]/2),
                     ifelse(ySclae == 'speakerScale', round(single_token$f0_speaker_max[1]-single_token$f0_speaker_range[1]/2),
                            ifelse(ySclae == 'dataScale', round(single_token$f0_data_max[1]-single_token$f0_data_range[1]/2), 250)))
...
print(plot_comp)
##--save?
ggsave(plot_comp, file=paste0("plots/",sel_file,"_PERIOGRAM+(",ySclae,").pdf"),device=cairo_pdf)
