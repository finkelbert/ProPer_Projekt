# ProPer analyses (IV): *Synchrony*, *Scaling* & *Mass*

Quantifying continuous prosody with periodic energy and F0 data to describe pitch contours (*Synchrony* and *Scaling*) and quantify prosodic strength (*Mass*).
...
CoM <- function(df) {
  # print(df)
  ifelse(
    round(sum(df$smogPP_20Hz, na.rm=T)) == 0, 0,
...
# comp_df <- mutate(group_by(comp_df, speaker),
...
comp_df <- mutate(
  group_by(comp_df,file),
                  ...
                  # f0_stim_range = f0_stim_max - f0_stim_min,
                  # f0_speaker_range = f0_speaker_max - f0_speaker_min,
  scaling = ifelse(
    auto_bounds==min(auto_bounds, na.rm=T), f0atCoM - f0_speaker_median, 
    ifelse(
      (!is.na(auto_bounds) & !is.na(f0atCoM)), scaling, NA)),
  scaling = ifelse(
    !is.na(AUC), na.locf(scaling, na.rm=F), NA),
  scale_rel = round((abs(scaling) / f0_speaker_range) * 100, 3),
  logScale_rel = round(log2(scale_rel+1)+1,3),
  scaleLabel = ifelse(
    round(scaling)>0, paste0('+',round(scaling),'Hz (',round(scale_rel),'%)'), 
    paste0(round(scaling),'Hz (',round(scale_rel),'%)')),
  #
  auto_boundsNum = length(which(!is.na(auto_bounds)))-1,
  # AUC_rel = round(AUC / (sum(smogPP_20Hz, na.rm=T) / syll_boundNum), 2),
  # AUC_rel = round(AUC / (sum(smogPP_20Hz, na.rm=T) / auto_boundsNum), 2),
  # AUC_rel = round(AUC / (sum(smogPP_20Hz, na.rm=T) / length(levels(as.factor(CoM)))), 2),
  sync_rel = round((abs(synchrony) / intervalDuration) * 100, 3),
  # logSync_rel = round(log2(sync_rel+1)+1,3),
  syncLabel = ifelse(
    round(synchrony)>0, paste0('+',round(synchrony),' ms (',round(sync_rel),'%)'), 
    paste0(round(synchrony),' ms (',round(sync_rel),'%)')),
  # massXsync = AUC_rel * logSync_rel,
  # massXscale = AUC_rel * logScale_rel,
  # massXf0 = massXscale + massXsync,
  CoM_corr = ifelse(CoM!=lag(CoM) | (!is.na(CoM) & is.na(lag(CoM))), CoM, NA),
  CoG_corr = ifelse(CoG!=lag(CoG) | (!is.na(CoG) & is.na(lag(CoG))), CoG, NA),
  #
  AUC_rel = round(AUC / (sum(smogPP_20Hz, na.rm=T) / length(levels(as.factor(CoM_corr)))), 2),
  intervalDuration_rel = ifelse(
    !is.na(intervalDuration), 
    round(intervalDuration / max(intervalDuration, na.rm=T), 3), NA),
                  # intervalDuration_rel = ifelse(t==CoM & !is.na(CoM), intervalDuration_rel, NA),
                  # intervalDuration_relInv = ifelse(!is.na(intervalDuration_rel), (intervalDuration_rel*-1) + 1, NA),
                  # intervalDuration_relInterp = round(pracma::interp1(t, intervalDuration_relInv), 1),
                  # intervalDuration_relInterpStretch = ifelse((is.na(intervalDuration_relInterp) & 
                  #                                   t<min(which(!is.na(intervalDuration_relInterp)))),
                  #                             intervalDuration_relInterp[min(which(!is.na(intervalDuration_relInterp)))],
                  #                             ifelse((is.na(intervalDuration_relInterp) & 
                  #                                       t>=max(which(!is.na(intervalDuration_relInterp)))),
                  #                                    intervalDuration_relInterp[max(which(!is.na(intervalDuration_relInterp)))], intervalDuration_relInterp)),
                  # intervalDuration_relSmoothInterp = bwfilter(wave = intervalDuration_relInterpStretch, f = sampleRate, to = 2, n = 1),
                  # intervalDuration_relSmoothInterp = ifelse(!is.na(intervalDuration_relInterp), intervalDuration_relSmoothInterp, NA),
                  #
  CoM_seq = na.locf(CoM, na.rm=F),
  CoM_diff = ifelse(
    !is.na(CoM_corr), CoM_corr - lag(CoM_seq, 2), NA),
  # CoM_diffBPM = 1000/CoM_diff*10,
  CoM_diff_rel = ifelse(
    !is.na(CoM_corr), round(CoM_diff / max(CoM_diff, na.rm=T), 3), NA),
  # CoM_diffBPM = ifelse(
  #   t == min(CoM_corr, na.rm=T), intervalDuration_rel, CoM_diffBPM),
  CoM_diff_rel = ifelse(
    t == min(CoM_corr, na.rm=T), intervalDuration_rel, CoM_diff_rel),
  # CoM_diff_rel = ifelse(
  #   CoM_corr == min(CoM_corr, na.rm=T), mean(CoM_diff_rel, na.rm=T),
  #   CoM_diff_rel),
  # CoM_diffBPM = na.locf(CoM_diffBPM, na.rm=F),
  CoM_diff_rel = na.locf(CoM_diff_rel, na.rm=F),
  # CoM_diffBPM = ifelse(
  #   t==CoM & !is.na(CoM), CoM_diffBPM, NA),
  CoM_diff_rel = ifelse(
    t==CoM & !is.na(CoM), CoM_diff_rel, NA),
  CoM_diff_relInv = ifelse(!is.na(CoM_diff_rel), (CoM_diff_rel*-1) + 1, NA),
  # CoM_diffBPMInterp = pracma::interp1(t, CoM_diffBPM),
  CoM_diff_relInterp = pracma::interp1(t, CoM_diff_relInv),
  #
  # CoM_diffBPMInterpStretch = ifelse(
  #   (is.na(CoM_diffBPMInterp) & t<min(which(!is.na(CoM_diffBPMInterp)))),
  #   CoM_diffBPMInterp[min(which(!is.na(CoM_diffBPMInterp)))],
  #   ifelse(
  #     (is.na(CoM_diffBPMInterp) & t>=max(which(!is.na(CoM_diffBPMInterp)))),
  #     CoM_diffBPMInterp[max(which(!is.na(CoM_diffBPMInterp)))], CoM_diffBPMInterp)),
  ...
  # CoM_diffBPMSmoothInterp = bwfilter(wave = CoM_diffBPMInterpStretch, f = 1000, to = 2, n = 1),
  CoM_diff_relSmoothInterp = bwfilter(wave = CoM_diff_relInterpStretch, f = 1000, to = 2, n = 1),
  # CoM_diffBPMSmoothInterp = ifelse(
  #   !is.na(CoM_diffBPMInterp), CoM_diffBPMSmoothInterp, NA),
  CoM_diff_relSmoothInterp = ifelse(
...
####################### Speech-rate curve
  geom_line(aes(y=CoM_diff_relSmoothInterp*f0_stim_range+plotFloor),color="springgreen4", alpha=.75, size=2.5, linetype="solid", linejoin = "round")+#, lineend = "butt", linejoin = "round") +
####################### boundaries
...
####################### annotations
  ...
  ##### Mass
  geom_text(aes(x=single_stim$CoM, y=plotFloor-plotUnits, label=AUC_rel), check_overlap=T, color="red", size=2, family= "Helvetica") +
####################### legend
  ##### CoGs
