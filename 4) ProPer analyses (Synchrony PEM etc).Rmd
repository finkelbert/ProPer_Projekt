...
Initial chunk deals with boundary detection -- either adjustable automatic detection ("auto") or an external manual segmentation (man")

```{r bound_detector, warning=FALSE, echo=FALSE}

detection_sensitivity <- 9  # choose values from 5 to 20 (low-pass smooth in Hz), 
                            # for coarse to fine automatic boundary detection
...
if(detetor_type == "auto") boundetect <- which(
     ...
     comp_df$smog_pp_2nd_der > 0.1 & 
     ## eliminate elbows:
     lead(comp_df$smog_pp_1st_der,25) > -0.1 &
     ## (continue)
...
##### Periodic energy (smog_ppXXHz)
  geom_line(aes(y=(smog_pp20Hz*(f0_stim_min))),color="red", alpha=.4, size=1.5) +
#### boundaries
...
##### Derivatives
  geom_line(aes(y=smog_pp_1st_der*2),color="black", alpha=.4, size=1) +
  geom_line(aes(y=smog_pp_2nd_der*2),color="gold", alpha=.8, size=1) +
##### annotations
  geom_text(aes(x=detect_single$interval_mid, y=f0_stim_min+10, label=as.character(label), check_overlap=T), size=4, color="black", family= "Avenir") + 
...
CoM <- function(df) {
  round(sum(df$smog_pp20Hz * df$t, na.rm=T) / sum(df$smog_pp20Hz, na.rm=T))
}
...
CoG <- function(df) {
  ...
  round(sum((df$f0_smooth_interp - f0Floor) * df$t * df$smog_pp20Hz, na.rm=T) / sum((df$f0_smooth_interp - f0Floor) * df$smog_pp20Hz, na.rm=T))
}
...
comp_df <- mutate(group_by(comp_df,file),
                  ...
                  AUC_rel = round(AUC / (round(round(sum(smog_pp20Hz, na.rm=T), 1) / length(levels(as.factor(CoM))), 1)), 2),
...
##### Periodic energy (smog_pp)
  geom_line(aes(y=(smog_pp20Hz*(f0_stim_min-50))+50),color="red", alpha=.5, size=.5) +
#### boundaries
...
#### annotations
  geom_text(aes(x=single_stim$interval_mid, y=f0_stim_max+30, label=label, check_overlap=T), color="black", size=5, family= "Avenir") +
