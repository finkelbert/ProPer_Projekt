...
if(length(files_textgrid)>0) tg_df <- plyr::ldply(files_textgrid, function(f){
  ...
  # t1 <- round(seg_tier$t1,3)*1000
  # t2 <- round(seg_tier$t2,3)*1000
  t_mid <- round((t1+t2)/2)
  label <- seg_tier$label
  data.frame(file, t=as.integer(as.character(t1)), seg_start=as.integer(as.character(t1)), seg_mid=as.integer(as.character(t_mid)), seg_end=as.integer(as.character(t2)), label)
})
if(length(files_textgrid)>0) tg_df$label <- levels(tg_df$label)[as.numeric(tg_df$label)]
# if(length(files_textgrid)>0) tg_df <- mutate(group_by(tg_df,file), seg_bounds = ifelse(seg_start>0, seg_start, NA))
