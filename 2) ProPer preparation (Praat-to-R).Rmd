...
if(length(files_textGrid)>0) textGridSyll_df <- plyr::ldply(files_textGrid, function(f){
  ...
  data.frame(file, syll_start=as.integer(as.character(t1)), syll_mid=as.integer(as.character(t_mid)), syll_end=as.integer(as.character(t2))-1, syll_bounds=as.integer(as.character(t1)), syll_label)
})
...
#
if(exists("textGridSyll_df")) textGridSyll_df <- dplyr::filter(textGridSyll_df, !is.na(syll_bounds))
...
if(length(files_textGrid)>0) textGridWord_df <- plyr::ldply(files_textGrid, function(f){
  ...
  data.frame(file, word_start=as.integer(as.character(word_t1)), word_mid=as.integer(as.character(word_t_mid)), word_end=as.integer(as.character(word_t2))-1, word_bounds=as.integer(as.character(word_t1)), word_label)
})
...
#
if(exists("textGridWord_df")) textGridWord_df <- dplyr::filter(textGridWord_df, !is.na(word_bounds))
...
# combine with textgrid annotations
...
raw_df <- droplevels(subset(raw_df, select = -file.y))#, syll_start, syll_mid, syll_end)))
