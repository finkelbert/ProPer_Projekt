...
intensity_df <- plyr::ldply(files_intensity, function(f){  
  ...
  time = round(intensitier$t,4)*1000
...
pitchObject_df <- plyr::ldply(files_pitchObject, function(f){  
  ...
  time <- round(pitch_object$t,4)*1000
...
if(length(files_textGrid)>0) textGridSyll_df <- plyr::ldply(files_textGrid, function(f){
  ...
  t1 <- ifelse(syll_tier$label=="", NA, round(syll_tier$t1,4)*1000)
  t2 <- ifelse(syll_tier$label=="", NA, round(syll_tier$t2,4)*1000)
  t_mid <- round((t1+t2)/2)
  syll_label <- syll_tier$label
  data.frame(file, syll_start=as.integer(as.character(t1)), syll_mid=as.integer(as.character(t_mid)), syll_end=as.integer(as.character(t2))-1, syll_label) #syll_bounds=as.integer(as.character(t1)), 
})
...
#   )
#
if(exists("textGridSyll_df")) textGridSyll_df <- dplyr::filter(textGridSyll_df, !is.na(syll_start))
...
if(length(files_textGrid)>0) textGridWord_df <- plyr::ldply(files_textGrid, function(f){
  ...
  data.frame(file, word_start=as.integer(as.character(word_t1)), word_mid=as.integer(as.character(word_t_mid)), word_end=as.integer(as.character(word_t2))-1, word_label)# word_bounds=as.integer(as.character(word_t1)), 
})
...
#   )
#
if(exists("textGridWord_df")) textGridWord_df <- dplyr::filter(textGridWord_df, !is.na(word_start))
...
# combine with textgrid intervals
if(exists("textGridSyll_df")) raw_df <- fuzzy_left_join(
  raw_df, textGridSyll_df,
  by = c("file" = "file", "t" = "syll_start", "t" = "syll_end"),
  match_fun = list(`==`, `>=`, `<=`)) 
# get rid of residues (.y)
if(exists("textGridSyll_df")) raw_df <- droplevels(subset(raw_df, select = -file.y))
# rename residual names (.x)
if(exists("textGridSyll_df")) raw_df <- raw_df %>%
  rename(file = file.x)
# add the syllable bounds
if(exists("textGridSyll_df")) raw_df <- mutate(
  group_by(raw_df,file),
  syll_bounds = ifelse(
    (is.na(syll_end) & !is.na(lag(syll_end,1))),
    lag(syll_end,1)+1, 
    syll_start)
  )
###############################
if(exists("textGridWord_df")) raw_df <- fuzzy_left_join(
  raw_df, textGridWord_df,
  by = c("file" = "file", "t" = "word_start", "t" = "word_end"),
  match_fun = list(`==`, `>=`, `<=`)) 
# get rid of residues (.y)
if(exists("textGridWord_df")) raw_df <- droplevels(subset(raw_df, select = -file.y))# -c(file.y, t.y)
# rename residual names (.x)
if(exists("textGridWord_df")) raw_df <- raw_df %>%
  rename(file = file.x)#, t = t.x)
# add the word bounds
if(exists("textGridWord_df")) raw_df <- mutate(
  group_by(raw_df,file),
  word_bounds = ifelse(
    (is.na(word_end) & !is.na(lag(word_end,1))),
    lag(word_end,1)+1, 
    word_start)
  )
