...
# TMPER: T Mark's PERiodic energy detector (native R functions by T Mark Ellison)
# alt name: Superiod

Code #1: Load functions to R (pre-package form) to obtain the raw periodic energy vector

```{r load_fun, warning=FALSE}
...
mkPeriodicEnergy <- function(wavFile, transform=(function(x) { x }), subSample=10, taus=(44:512), log2WindowSize=11) {
  ...
  df <- data.frame(ms=round(x,2), sample=(1:l), osc=an, sr=a@samp.rate)
  autocorrelateMeanSinusoidPower( ac ) %>% sqrt() %>% normalise() -> df$w
  df$pe <- round(transform( df$w ), 3)
  ...
  #   geom_line(data=dfa,aes(x=x,y=(y-1)*.2),colour="black",size=0.3) +
  #   theme_bw() +
