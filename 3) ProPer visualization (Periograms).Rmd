...
LowPass6Hz <- butter(n = 1, W = 6/(1000/2), type = "low")
## F0 interpolating and smoothing: mutate by token
main_df <- mutate(
  ...
  ##  f0_smooth_stretch_interp
  f0_smootcherp = na.spline(f0_smootch, na.rm=F),
  ##  f0_smooth_stretch_interp_smooth
  f0_smootcherpooth = signal::filter(LowPass6Hz, f0_smootcherp),
  ##  f0_smooth_stretch_interp_smooth_drop outer stretch
  f0_ed_dropStr = ifelse(
  is.na(na.approx(f0_smooth, na.rm=F)), NA, f0_smootcherpooth),
  ##  f0_smooth_stretch_interp_smooth_drop interpolation
  f0_ed_dropPol = ifelse(
  is.na(f0_smooth), NA, f0_smootcherpooth)
  )
...
main_df <- mutate(
  ...
  f0_data_min = round(min(f0_ed_dropPol, na.rm=T)),
  f0_data_max = round(max(f0_ed_dropPol, na.rm=T)),
...
main_df <- mutate(
  ...
  f0_token_min = round(min(f0_ed_dropPol, na.rm=T)),
  f0_token_max = round(max(f0_ed_dropPol, na.rm=T)),
  f0_token_median = round(median(f0_ed_dropPol, na.rm = T)),
  f0_token_mean = round(mean(f0_ed_dropPol, na.rm = T)),
...
if(length(main_df$speaker)>0) main_df <- mutate(group_by(main_df, speaker),
  ...
  f0_speaker_min = round(min(f0_ed_dropPol, na.rm=T)),
  f0_speaker_max = round(max(f0_ed_dropPol, na.rm=T)),
  f0_speaker_min_median = round(median(f0_token_min, na.rm=T)),
  f0_speaker_max_median = round(median(f0_token_max, na.rm=T)),
  f0_speaker_median = round(median(f0_ed_dropPol, na.rm = T)),
  f0_speaker_mean = round(mean(f0_ed_dropPol, na.rm = T)),
...
LowPass12Hz <- butter(n = 1, W = 12/(1000/2), type = "low")
LowPass8Hz <- butter(n = 1, W = 8/(1000/2), type = "low")
LowPass5Hz <- butter(n = 1, W = 5/(1000/2), type = "low")
#
########## periodic energy smoothing (log+smooth = smog)
main_df <- mutate(
  group_by(main_df, file),
  ### 20Hz low-pass filter (50ms intervals): "segmental smooth"
  smogPP_20Hz = signal::filter(LowPass20Hz, logPP),
  ### 12Hz low-pass filter (~83.3ms  intervals): "seg-syll smooth"
  smogPP_12Hz = signal::filter(LowPass12Hz, logPP),
  ### 8Hz low-pass filter (125ms  intervals): "syll-seg smooth"
  smogPP_8Hz = signal::filter(LowPass8Hz, logPP),
  ### 5Hz low-pass filter (200ms  intervals): "syllabic smooth"
  smogPP_5Hz = signal::filter(LowPass5Hz, logPP)
  )
...
## periogram (smogPP)
  geom_line(aes(y=f0_smootcherpooth),color="magenta2", alpha=single_token1$smogPP_20Hz, size=single_token1$smogPP_20Hz*5) +
#
########## Power/intensity
## intensity
...
########## F0 curves
## periogram (smogPP)
  geom_line(aes(y=f0_smootcherpooth),color="magenta2", alpha=single_token2$smogPP_20Hz, size=single_token2$smogPP_20Hz*5) +
########## Periodic power 'pp' (total power * periodic fraction)
