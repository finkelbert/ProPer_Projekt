...
main_df <- mutate(
  ...
  f0_stim_median = round(median(f0_smooth, na.rm = T),2),
  f0_stim_mean = round(mean(f0_smooth, na.rm = T),2),
  f0_stim_range = round(f0_stim_max - f0_stim_min, 2),
  # variables for plot normalization
  plotFloor = round(f0_stim_min - f0_stim_range, 2),
  ## conclude relative anchors and thresholds
  perFloorFix = perFloor, # keep records for potential fix
  perFloor_indeed = ifelse(
    relTo=="stim", round(max_stim_per_power * perFloor, 10), ifelse(
      relTo=="data", round(max_data_per_power * perFloor, 10), 
      round(max_speaker_per_power * perFloor, 10))),
  strengThresh_indeed = ifelse(
    relTo=="stim", round(max_stim_strength * strengThresh, 8), ifelse(
...
########## Periodic power 'pp' (total power * periodic fraction)
  geom_line(aes(y=postPP_rel*f0_stim_range+plotFloor),color="purple3", alpha=.5, size=.5, linetype="solid") +
...
########## Smoothed logPP 'smogPP' (4 smoothing flavors: 5/ 8/ 12/ 20 Hz low-pass filter)
  geom_line(aes(y=smogPP_20Hz*f0_stim_range+plotFloor),color="lightsteelblue", alpha=.5, size=.75) +
...
## re-adjust selected stimuli: change the perFloor value of specific stimuli
...
main_df <- mutate(
  ...
  #### re-run 
  perFloor_indeed = ifelse(
    relTo=="stim", round(max_stim_per_power * perFloorFix, 10), ifelse(
