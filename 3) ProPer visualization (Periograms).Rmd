...
setwd("~/repos/ProPer_Projekt3/ProPer_Projekt/")

source("functions.R")

## Read the previous table: raw_df
raw_df <- praat_to_table("test/textgrids/")
...
main_df <- main_df |>
  group_by(file) |>
  mutate(totalPower = round(4e-10 * 10^(intensity_dB / 10), 9),
         periodicPower = round(totalPower * periodicity_index, 9))
...
TODO: Extract variables from file names!
...
main_df <- main_df |>
  group_by(file) |>
  mutate(
    f0_smerp = na_interpolation(f0_Hz, option = "stine"),
    f0_smoorp = round(
      bwfilter(wave = f0_smerp,
               f = 1000,
               to = 6,
               n = 1,
               output = "Sample"), 
...
main_df <- main_df |>
  group_by(file) |>
  mutate(
    periodic_energy = get_periodic_energy(
      intensity_dB,
      periodicity_index,
      perfloor = 0.01,
      threshold = 0.25
...
```{r}
ggplot(main_df |> filter(file == "AH_1"), aes(x = time, y = periodicity_index)) +
  geom_path()
```

```{r}
ggplot(main_df |> filter(file == "AH_1"), aes(x = time, y = periodic_energy)) +
  geom_path()
```
```{r}
hz_to_ome <- function(x) {
...
main_df <- main_df |>
  group_by(file) |>
  mutate(f0_OMe = hz_to_ome(f0_Hz)) |>
  ungroup()
```

```{r}
ggplot(main_df |> filter(file == "AH_1"), aes(x = time, y = f0_Hz)) +
  geom_path()  +
  scale_y_continuous(limits = c(100, 300))

ggplot(main_df |> filter(file == "AH_1"), aes(x = time, y = f0_OMe)) +
