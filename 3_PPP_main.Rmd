...
## threshold for the raw strength data from the pitch object (not applicable to row1/f0-related data).
strengthresh <- 0.35
#
...
## per_floor and strengthresh are calculated relative to the maximum values of either the wole set (same recording session and speaker), per speaker, or for each stimulus (different recording conditions). 
stim_data <- "stim"
#
################-------- Smoothers
...
max_data_per_power <- round(max(main_df$periodic_power_freq_lim, na.rm=T),7)
...
#                   max_speaker_per_power = max(periodic_power_freq_lim, na.rm=T),
...
main_df <- mutate(group_by(main_df,file),
                  ## per_floor threshold settings
                  max_stim_per_power = max(periodic_power_freq_lim, na.rm=T),
                  per_thresh_indeed = ifelse(stim_data=="stim", max_stim_per_power * per_floor, ifelse(stim_data=="speaker", max_speaker_per_power * per_floor, max_data_per_power * per_floor)),
                  ## strengthresh threshold settings
                  max_stim_strength = max(strength_rowmax, na.rm=T),
                  strengthresh_indeed = ifelse(stim_data=="stim", max_stim_strength * strengthresh, ifelse(stim_data=="speaker", max_speaker_strength * strengthresh, max_data_strength * strengthresh)),
                  ...
                  log_pp_gnrl = 10*log10(periodic_power_general/per_thresh_indeed),
                  log_pp_gnrl = ifelse(log_pp_gnrl<0, 0, log_pp_gnrl),
                  smog_pp_gnrl = round(predict(loess(log_pp_gnrl~t, data=main_df$file, span=per_smooth_span, degree = 1, na.action = "na.exclude")),7),
                  smog_pp_gnrl = ifelse(smog_pp_gnrl<0, 0, smog_pp_gnrl),
                  ## log and smooth f0-related version (using Praat's f0 path finder thresholds)
                  log_pp_f0 = 10*log10(periodic_power_f0/per_thresh_indeed),
                  log_pp_f0 = ifelse(log_pp_f0<0, 0, log_pp_f0),
                  ## smooth logged periodic power f0-related (smog_pp_f0)
                  smog_pp_f0 = round(predict(loess(log_pp_f0~t, data=main_df$file, span=per_smooth_span, degree = 1, na.action = "na.exclude")),7),
                  smog_pp_f0 = ifelse(smog_pp_f0<0, 0, smog_pp_f0),
                  ...
                  periodic_power_f0_rel = ifelse(periodic_power_f0<0, 0, round(periodic_power_f0/max(periodic_power_f0, na.rm=TRUE),7)),
                  log_pp_gnrl_rel = round(log_pp_gnrl/max(log_pp_gnrl, na.rm=TRUE),7),
                  log_pp_f0_rel = round(log_pp_f0/max(log_pp_f0, na.rm=TRUE),7),
                  smog_pp_gnrl_rel = round(smog_pp_gnrl / max(smog_pp_gnrl, na.rm = T),7),
                  smog_pp_f0_rel = round(smog_pp_f0/max(smog_pp_f0, na.rm=TRUE),7),
                  ...
                  ## get 1st and 2nd derivatives from periodic energy
                  smog_pp_1st_derive =  ifelse(!is.na(smog_pp_gnrl), diff(lag(smog_pp_gnrl,1)), 0),
                  smog_pp_1st_derive = round(predict(loess(smog_pp_1st_derive~t, data=main_df$file, span=0.1, degree = 1, na.action = "na.exclude")),7),
                  smog_pp_2nd_derive =  ifelse(!is.na(smog_pp_1st_derive), diff(lag(smog_pp_1st_derive,1))*1000, 0),
                  smog_pp_2nd_derive = round(predict(loess(smog_pp_2nd_derive~t, data=main_df$file, span=0.1, degree = 1, na.action = "na.exclude")),7),
                  per_fluct_bound = NA,
                  scaling = NA
)
...
plot_all <- 
  ...
  geom_line(aes(y=f0_smooth_interp),color="magenta2", alpha=main_df$smog_pp_gnrl_rel, size=main_df$smog_pp_gnrl_rel*3) + 
  ## periogram_f0 (smog_pp_f0)
  #geom_line(aes(y=f0_smooth_interp),color="magenta2", alpha=main_df$smog_pp_f0_rel, size=main_df$smog_pp_f0_rel*3) + 
  ####### Periodic energy
  ## general periodic energy (smog_pp_gnrl)
  geom_line(aes(y=smog_pp_gnrl_rel*f0_stim_min),color="gold", alpha=.5, size=1) + 
  ## F0-related periodic energy (smog_pp_f0)
  #geom_line(aes(y=smog_pp_f0_rel*f0_stim_min),color="yellow", alpha=.5, size=1) + 
...
plot1 <- 
  ...
  ########## Periograms
  ## periogram_general (smog_pp_gnrl)
  geom_line(aes(y=f0_smooth_interp),color="magenta2", alpha=single_stim$smog_pp_gnrl_rel, size=single_stim$smog_pp_gnrl_rel*5) + 
  ## periogram_f0-related (smog_pp_f0)
  #geom_line(aes(y=f0_smooth_interp),color="magenta2", alpha=single_stim$smog_pp_f0_rel, size=single_stim$smog_pp_f0_rel*5) + 
  ########## Power/intensity
  ## intensity
  #geom_line(aes(y=int_rel*f0_stim_min),color="green", alpha=.2, size=.5) + 
  ## power
  geom_line(aes(y=total_power_rel*f0_stim_min),color="cornflowerblue", alpha=.5, size=.5) + 
  ########## Periodic fraction (strength/HNR)
  ## raw general/freq_lim strength (before "strengthresh")
  #geom_line(aes(y=strength_rowmax*f0_stim_min), color="green", alpha=.3, size=.5) +
  ## general strength 
  geom_line(aes(y=strength_general*f0_stim_min), color="tomato", alpha=.5, size=.5) +
  ## f0-related strength
  #geom_line(aes(y=strength_f0*f0_stim_min), color="orange", alpha=.5, size=.5) +
  ########## Periodic power (total power * periodic fraction)
  ## general periodic power
  #geom_line(aes(y=periodic_power_general_rel*f0_stim_min),color="purple3", alpha=.6, size=.5) + 
  ## f0-related periodic power
  #geom_line(aes(y=periodic_power_f0_rel*f0_stim_min),color="purple1", alpha=.6, size=.5) + 
  ########## Log periodic power (10*log10(x/per_thresh))
  ## general log pp
  #geom_line(aes(y=log_pp_gnrl_rel*f0_stim_min),color="white", alpha=.5, size=.5) + 
  ## f0-related log pp
  #geom_line(aes(y=log_pp_f0_rel*f0_stim_min),color="yellow", alpha=.5, size=.5) + 
  ########## Smoothed log_pp (final mutation)
  ## general smog pp
  #geom_line(aes(y=smog_pp_gnrl_rel*f0_stim_min),color="gold", alpha=.4, size=2) + 
  ## f0-related smog pp
  geom_line(aes(y=smog_pp_f0_rel*f0_stim_min-10),color="gold", alpha=.5, size=1.5) + 
