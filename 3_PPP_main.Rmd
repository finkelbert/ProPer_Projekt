...
main_df <- mutate(group_by(main_df,file),
                  ...
                  smog_pp_1st_derive =  ifelse(!is.na(smog_pp), diff(lag(smog_pp,1)), 0),
...
####### Periodic energy (smog_pp)
  geom_line(aes(y=smog_pp_rel*f0_stim_min),color="gold", alpha=.5, size=1) + 
...
########## Log periodic power (10*log10(x/per_thresh))
  geom_line(aes(y=log_pp_rel*f0_stim_min),color="white", alpha=.5, size=.5) +
########## Smoothed log_pp (final mutation)
  geom_line(aes(y=smog_pp_rel*f0_stim_min),color="gold", alpha=.4, size=2) +
